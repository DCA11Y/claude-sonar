...
const SONAR_COMMAND_PREFIX = "claude-sonar format";

/** All hook event types sonar registers for. */
...
/**
 * Build the sonar hook entry for a given event type.
 */
function buildSonarHook(): HookEntry {
  return {
    matcher: "",
    hooks: [
      {
        type: "command",
        command: SONAR_COMMAND_PREFIX,
      },
    ],
  };
}
...
/**
 * Structurally detect if a hook entry is a sonar hook.
 * Checks the command prefix, not JSON.stringify substring match.
 */
function isSonarHook(entry: unknown): boolean {
  ...
  return (obj["hooks"] as unknown[]).some((h) => {
    ...
    return (
      ...
      (hook["command"] as string).startsWith(SONAR_COMMAND_PREFIX)
    );
  });
}

/**
 * Check if sonar hooks are already installed.
 * Checks PostToolUse for backward compatibility.
 */
export function isSonarInstalled(settingsPath?: string): boolean {
  ...
  return postToolUse.some(isSonarHook);
}

/**
 * Install sonar hooks for all supported event types into Claude Code settings.
...
export function installHooks(settingsPath?: string): string {
  ...
  for (const eventType of HOOK_EVENT_TYPES) {
    ...
    const existingIdx = eventHooks.findIndex(isSonarHook);
    const newHook = buildSonarHook();
  ...
  if (updated && !installed) {
    return "Updated existing sonar hooks.";
  }
  return "Installed sonar hooks.";
}

/**
 * Remove sonar hooks from all event types in Claude Code settings.
 * Returns a description of what was done.
 */
export function removeHooks(settingsPath?: string): string {
  ...
  for (const eventType of HOOK_EVENT_TYPES) {
    ...
    const filtered = eventHooks.filter((entry) => !isSonarHook(entry));
    totalRemoved += before - filtered.length;
    hooks[eventType] = filtered;
  }

  if (totalRemoved === 0) {
    return "No sonar hooks found. Nothing to remove.";
  }

  writeSettings(settings, p);
  return `Removed ${totalRemoved} sonar hook(s).`;
}
