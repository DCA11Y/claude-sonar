...
export function getConfigDir(): string {
  if (process.env["CLAUDE_SONAR_CONFIG_DIR"]) {
    return process.env["CLAUDE_SONAR_CONFIG_DIR"];
  }
  const xdg = process.env["XDG_CONFIG_HOME"] || path.join(homedir(), ".config");
  return path.join(xdg, "claude-sonar");
}
...
export function loadConfig(): SonarConfig {
  try {
    ...
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
      return structuredClone(DEFAULT_CONFIG) as SonarConfig;
    }
 ...
 */
function mergeConfig(user: Record<string, unknown>): SonarConfig {
...
function isValidVerbosity(v: unknown): v is SonarConfig["verbosity"] {
  return v === "compact" || v === "minimal" || v === "normal" || v === "full";
}

function isValidEngine(v: unknown): v is SonarConfig["tts"]["engine"] {
  return v === "auto" || v === "say" || v === "spd-say";
}

function isValidEarconEngine(v: unknown): v is SonarConfig["earcon"]["engine"] {
...
export function resetConfig(): void {
  writeConfig(structuredClone(DEFAULT_CONFIG) as SonarConfig);
}

/**
 * Write config atomically (write-to-temp-then-rename).
 */
function writeConfig(config: SonarConfig): void {
