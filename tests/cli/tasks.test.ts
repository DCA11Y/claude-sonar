...
import { saveTaskSnapshot } from "../../src/core/task-tracker.js";
import { tasksCommandNonInteractive } from "../../src/cli/commands/tasks.js";

let tmpDir: string;

beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "sonar-tasks-cli-test-"));
...
describe("tasksCommandNonInteractive", () => {
  ...
  it("handles corrupted snapshot gracefully", () => {
    const taskDir = path.join(tmpDir, "claude-sonar", "tasks");
