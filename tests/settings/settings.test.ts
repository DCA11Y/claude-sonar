...
describe("settings", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "sonar-settings-"));
  ...
  describe("installHooks", () => {
    it("creates settings with hooks for all event types when no file exists", () => {
      ...
      expect(settings.hooks.PostToolUse[0].hooks[0].command).toContain("claude-sonar format");
      expect(settings.hooks.Notification[0].hooks[0].command).toContain("claude-sonar format");
      expect(settings.hooks.PermissionRequest[0].hooks[0].command).toContain("claude-sonar format");
    });
    ...
    it("preserves existing non-sonar hooks", () => {
      writeSettings(
        {
          hooks: {
            PostToolUse: [
    ...
    it("updates existing sonar hooks (upgrade path)", () => {
      installHooks(settingsPath);
      const result = installHooks(settingsPath);
  ...
  describe("removeHooks", () => {
    ...
    it("removes sonar hooks from all event types and preserves others", () => {
      writeSettings(
        {
          hooks: {
            PostToolUse: [
              { matcher: "", hooks: [{ type: "command", command: "other-tool" }] },
              { matcher: "", hooks: [{ type: "command", command: "claude-sonar format" }] },
            ],
            Notification: [
              { matcher: "", hooks: [{ type: "command", command: "claude-sonar format" }] },
            ],
    ...
    it("returns message when no sonar hooks found", () => {
      writeSettings(
        {
          hooks: {
            PostToolUse: [
      ...
      const result = removeHooks(settingsPath);
      expect(result).toContain("No sonar hooks found");
    });
  });

  describe("isSonarInstalled", () => {
    it("returns false when no settings file", () => {
      expect(isSonarInstalled(settingsPath)).toBe(false);
    });

    it("returns true after install", () => {
      installHooks(settingsPath);
      expect(isSonarInstalled(settingsPath)).toBe(true);
    });
