...
import { recordAndSequence, cleanStaleSessions } from "../../src/core/sequencer.js";
...
describe("sequencer", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "sonar-seq-"));
  ...
  describe("recordAndSequence", () => {
    ...
    it("sanitizes session IDs for file safety", () => {
      ...
      const sessionDir = path.join(tmpDir, "claude-sonar", "sessions");
