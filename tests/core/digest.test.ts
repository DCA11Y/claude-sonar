...
import type { DigestEntry } from "../../src/core/digest.js";

let tmpDir: string;

beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "sonar-digest-test-"));
...
describe("appendToDigest + flushDigest", () => {
  ...
  it("handles corrupted digest file", () => {
    const digestDir = path.join(tmpDir, "claude-sonar", "digests");
