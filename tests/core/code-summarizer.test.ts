...
import {
  ...
  type Declaration,
  type CodeSummary,
} from "../../src/core/code-summarizer.js";
...
export async function loadConfig(): Promise<SonarConfig> {
  return {};
}
`;
    ...
    expect(result.declarations[1]).toMatchObject({
      ...
      name: "loadConfig",
      params: "()",
      returnType: "Promise<SonarConfig>",
      exported: true,
      async: true,
    });
  });
  ...
  it("extracts const declarations", () => {
    const code = `export const DEFAULT_CONFIG: SonarConfig = {};`;
    const result = summarizeCode(code, "defaults.ts");
    expect(result.declarations).toHaveLength(1);
    expect(result.declarations[0]).toMatchObject({
      kind: "const",
      name: "DEFAULT_CONFIG",
      returnType: "SonarConfig",
      exported: true,
    });
  });
...
describe("formatDeclaration", () => {
  ...
  it("formats const with type", () => {
    ...
    expect(formatDeclaration(decl)).toBe("export const DEFAULT: SonarConfig");
  });
});
